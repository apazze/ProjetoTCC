...
const char* nomeDoArquivo = "/Exemplo.txt";
...
void setup()
{
  ...
  EditarArquivo(SD, nomeDoArquivo, GeraMsg().c_str());
  delay(1000);
  LeArquivo();
}
void loop()
...
String GeraMsg()
{
  float value = random(10.0, 1000.0);
  String msg = "Bem vindo! Valor aleatorio [ " + String(value) + " ]";
  return msg;
}
...
void LeArquivo()
{
  ...
  File file = SD.open(nomeDoArquivo); // abre o arquivo para a leitura
  if (!file)
